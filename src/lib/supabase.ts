 ...
 *  degrade gracefully. Booking writes go through the confirm_booking()
 *  Postgres RPC (SECURITY DEFINER), so the anon client is sufficient
 *  even for the Stripe webhook — no service role key required. */
